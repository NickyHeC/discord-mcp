...
Provides direct HTTP access to Discord API version 9 using Dedalus dispatch.
"""
...
async def discord_api_request(
    ...
    """
    Make a request to Discord REST API v9 using Dedalus dispatch.
    ...
    # Build path with query params if needed
    path = endpoint
    if params:
    ...
    if resp.success:
        return resp.response.body or {}
    raise ValueError(resp.error.message if resp.error else "Discord request failed")
...
async def get_current_user_guilds_v9(limit: int = 200) -> list[Dict[str, Any]]:
    ...
    return await discord_api_request("GET", "/users/@me/guilds", params={"limit": min(limit, 200)})
